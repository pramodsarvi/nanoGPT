...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_triton = getattr(config, 'use_triton', False)
    ...
    def forward(self, x, kv_cache=None):
        ...
        if HAS_TRITON and self.use_triton and is_first_pass:
            # use triton attention kernel (only for non-KV cache path for now)
            y = triton_attention(q, k, v, causal=True)
            y = y.to(x.dtype)
        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        elif self.flash:
            # efficient attention using Flash Attention CUDA kernels
            # if we have kv_cache, it's not strictly causal in the sense of a triangle mask on the input,
            # but we only care about the last token's output.
...
class GPTConfig:
    ...
    use_triton: bool = False # use Triton kernels for LayerNorm and GELU
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, kv_caches=None):
        ...
        # If we have kv_caches, we are only processing the NEW tokens
        # The position is the total length so far
        if kv_caches is not None:
            # kv_caches is a list of (k, v) tuples, one per layer
            # Each k/v has shape (b, nh, seqlen, hs)
            past_seq_len = kv_caches[0][0].size(2)
            pos = torch.arange(past_seq_len, past_seq_len + t, dtype=torch.long, device=device)
        else:
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            x, new_block_kv_cache = block(x, kv_cache=block_kv_cache)
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, use_cache=True):
        ...
        # Fast behavior with KV caching
        kv_caches = None
        curr_idx = idx
        
        for i in range(max_new_tokens):
            # if it's the first iteration, we pass the whole prompt
            # otherwise, we only pass the last generated token
            if i == 0:
                idx_cond = curr_idx
            else:
                idx_cond = curr_idx[:, [-1]]
                
            # crop if necessary (though with KV cache we normally handle this differently, 
            # for nanoGPT simplicity we just ensure we don't exceed block size total)
            total_len = (kv_caches[0][0].size(2) if kv_caches is not None else 0) + idx_cond.size(1)
            if total_len > self.config.block_size:
                # If we exceed block size, we fall back to slow mode or just crop.
                # Here we just crop the start of the KV cache if it was possible, 
                # but for simplicity let's just break or warn.
                break
